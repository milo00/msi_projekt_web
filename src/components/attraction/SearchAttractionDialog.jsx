...
export const SearchAttractionDialog = ({
  open,
  onClose,
  dayPlanId,
  onSuccess,
}) => {
  const [selectAttractionDialogOpen, setSelectAttractionDialogOpen] =
    useState(false);
  ...
  const handleSearch = async (attractionName) => {
    setLoading(true);
    doGet(
      "/api/v1/attraction/find?" +
        new URLSearchParams({ name: attractionName }).toString()
    )
  ...
  return (
    <div>
      <ErrorToast
        ...
        message="Unable to search for no attraction!"
      />
      ...
      >
        ...
        >
          <Typography sx={{ fontSize: "32px" }}>Search attractions</Typography>
          ...
          {loading ? (
            ...
            >
              <Typography variant="h5">
                Search attractions and add them to day plan
              </Typography>
            </Box>
          ) : (
            <Box>
                ...
                {candidates.map((candidate) => (
                  ...
                  >
                    <AttractionCandidateCard
                      attractionData={candidate}
                      openSelectAttractionDialog={() =>
                        handleSelectAction(candidate)
                      }
                    />
                  </ListItem>
                ))}
