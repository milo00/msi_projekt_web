...
export const DayPlanPage = (props) => {
  ...
  const [searchAttractionDialogOpen, setSearchAttractionDialogOpen] =
    useState(false);
  const [
    searchAttractionNearbyDialogOpen,
    setSearchAttractionNearbyDialogOpen,
  ] = useState(false);
  ...
  const [deleteDayPlanConfirmToastOpen, setDeleteDayPlanConfirmToastOpen] =
    useState(false);
  const [
    deleteAttractionConfirmToastOpen,
    setDeleteAttractionConfirmToastOpen,
  ] = useState(false);
  ...
  const getData = async () => {
    setLoading(true);
    doGet(
      "/api/v1/day-plan?" + new URLSearchParams({ groupId: groupId }).toString()
    )
      .then((response) => response.json())
      .then((json) => {
        setdayPlansRaw(json);
  ...
  const updateDayplanAttractions = async () => {
    ...
    console.log("update");
    var newAttractions = await doGet(
      "/api/v1/attraction?" +
        new URLSearchParams({
          groupId: groupId,
    ...
    var dayPlanData = dayPlansRaw.find(
      (dayPlan) => dayPlan.dayPlanId === selectedDayPlanId
    );
    dayPlanData.dayAttractions = newAttractions;
    setdayPlansRaw(
      dayPlansRaw.map((dp) =>
        dp.dayPlanId === selectedDayPlanId ? dayPlanData : dp
      )
    );
    showDetailedPlan(
      dayPlanData.name,
      dayPlanData.date,
      dayPlanData.dayAttractions,
      dayPlanData.dayPlanId
    );
    setAllDayPlans(
      dayPlansRaw.map((dayPlan) => (
        <ListItem sx={{ p: 0, my: 1 }} key={dayPlan.dayPlanId}>
          <DayPlanCard
            dayPlanData={dayPlan}
            groupId={groupId}
            ...
            >
              ...
              >
                  ...
                  {attraction.distanceToNextAttraction ? (
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {(attraction.distanceToNextAttraction / 1000)
                            .toString()
                            .substring(0, 4)}{" "}
                          km
                        </Typography>
                ...
                <AttractionCard
                  ...
                  onDeletion={(selectedDayPlanId) =>
                    getDataAfterDeleteAttraction(selectedDayPlanId)
                  }
                />
              </Box>
            </ListItem>
              ...
              <Grid item xs={10} md={4}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      <FormatListBulletedIcon sx={{ fontSize: "32px" }} />
                        ...
                        >
                          <Tooltip
                            title={
                              ...
                              "Please allow sharing your location to add nearby attractions"
                            }
                          >
                            <span>
                              <Button
                                ...
                                onClick={() =>
                                  setSearchAttractionNearbyDialogOpen(true)
                                }
                              >
                                <AddIcon />
                                Add nearby
