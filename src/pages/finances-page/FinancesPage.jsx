...
export const FinancesPage = () => {
  ...
  const [addExpenditureDialogOpen, setAddExpenditureDialogOpen] =
    useState(false);
  ...
  const getBalanceData = async (userList) => {
    await doGet(
      "/api/v1/finance-optimizer/balance?" +
  ...
  const getAllUsersInGroup = async () => {
    await doGet(
      "/api/v1/user-group/participants?" +
        new URLSearchParams({ groupId: groupId }).toString()
    )
      .then((response) => response.json())
      .then((response) => {
        ...
        const person = response.map((user) => ({
          id: user.userId,
    ...
    await doGet(
      ...
      .then((response) => {
        var set = response.map((settlement) => {
          const debtor = userList.find(
            (user) => user.id === settlement.debtor
          ).fullName;
          const debtee = userList.find(
            (user) => user.id === settlement.debtee
          ).fullName;
          const debtorId = settlement.debtor;
          const debteeId = settlement.debtee;

          return {
            id: settlement.financialRequestId,
  ...
  const getExpendituresData = async (userList) => {
    await doGet(
      ...
      .then((response) => {
        setExpendituresData(
          response.map((expenditure) => {
            const person = userList.find(
              (user) => user.id === expenditure.creatorId
            ).fullName;
            const isDebtor = expenditure.expenseDebtors.some(
              (debtor) => debtor === parseInt(sessionStorage.getItem("userId"))
            );
            const contributors = expenditure.expenseDebtors.map((ed) => {
              return {
                id: userList.find((user) => user.id === ed).id,
                name: userList.find((user) => user.id === ed).fullName,
              };
            });
            return {
              id: expenditure.expenditureId,
              personId: expenditure.creatorId,
              person: person,
              title: expenditure.title,
              cost: expenditure.price,
              date: parseISO(expenditure.generationDate),
              isDebtor: isDebtor,
  ...
  const showUsersExpenditures = () => {
    ...
    if (myExpendituresButtonOn) {
      ...
      setAllExpenditures(
        expendituresData
          .filter((expenditure) => expenditure.personId === currentUser.userId)
          .map((expenditure) => (
            <ListItem sx={{ p: 0, my: "10px" }} key={expenditure.id}>
              <ExpenditureCard expenditureData={expenditure} />
            </ListItem>
  ...
  const showUsersContributions = () => {
    ...
    if (myContributionsButtonOn) {
      ...
      setAllExpenditures(
        expendituresData
          .filter((expenditure) =>
            expenditure.contributors.some(
              (debtor) => debtor.id === currentUser.userId
            )
          )
          .map((expenditure) => (
            <ListItem sx={{ p: 0, my: "10px" }} key={expenditure.id}>
    ...
    <>
      ...
      >
        <NavigationNavbar
          buttonsData={
            groupStage === 2
              ? currentTripButtonsDataWithGroupId(groupId)
              : pastTripButtonsData
          }
          groupId={groupId}
        />
                  ...
                  >
                    ...
                    >
                      <Button
                        sx={{
                          ...
                          fontSize: "15px",
                          mr: 2,
                          color: myExpendituresButtonOn
                            ? "#FFFFFF"
                            : "primary.main",
                          backgroundColor: myExpendituresButtonOn
                            ? "primary.main"
                            : "#FFFFFF",
                          "&:hover": {
                            backgroundColor: myExpendituresButtonOn
                      ...
                      <Button
                        sx={{
                          borderRadius: "20px",
                          fontSize: "15px",
                          color: myContributionsButtonOn
                            ? "#FFFFFF"
                            : "primary.main",
                          backgroundColor: myContributionsButtonOn
                            ? "primary.main"
                            : "#FFFFFF",
                          "&:hover": {
                            backgroundColor: myContributionsButtonOn
                    ...
                    >
                      {allExpenditures.length === 0 ? (
                        ...
                        >
                          <Typography variant="h6">Add expenditures</Typography>
                        </Box>
                      ) : (
                        <>
                          <List
